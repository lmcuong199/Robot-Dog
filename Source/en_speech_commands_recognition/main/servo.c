    ...
    // Per-leg mechanical trim, found by centering each servo at 90 and adjusting
    // the raw output until the horn looked level. Applied after the FR/BL inversion below.
    #define FL_TRIM 9
    #define FR_TRIM -26
    #define BL_TRIM 7
    #define BR_TRIM 33
    ...
    // converts an angle to PWM duty cycle for standard hobby servos 
    // (converts to a pulse width between SERVO_MIN_US and SERVO_MAX_US, then scales to 14-bit duty cycle)
    ...
    {
        cur.fl = deg;
        // clamps 0-180 and pushes duty to the LEDC (ESP32's PWM peripheral) channel
        servo_set_angle(SERVO_FL_CH, deg + FL_TRIM);
    }
    static void set_fr(int deg)
    {
        cur.fr = deg;
        // 180-deg to invert the angle because the right-side and left-side servos are mounted mirrored,
        // so the same "logical" angle needs opposite physical rotation to move symmetrically
        servo_set_angle(SERVO_FR_CH, (180 - deg) + FR_TRIM);
    }
    static void set_bl(int deg)
    {
        cur.bl = deg;
        servo_set_angle(SERVO_BL_CH, (180 - deg) + BL_TRIM);
    }
    static void set_br(int deg)
    {
        cur.br = deg;
        servo_set_angle(SERVO_BR_CH, deg + BR_TRIM);
    }
    ...
    // it also moves all four legs from current pos to a target over duration_ms, using a smoothstep easing curve (t*t*(3-2t)) in 20 steps, so movements isn't jerky 
    // ---------------------------------------------------------------------------
    static void ease_to(legs_t to, int duration_ms)
    {
        ...
        for (int i = 0; i <= steps; i++)
        {
            ...
            float e = t * t * (3.0f - 2.0f * t); // make the movement smooth, near start and end is slow but between is fast (s-curved)
    ...
    // a tail-wag-like oscillation using bl/br only, amplitude decaying over time
    ...
    // front dips, rear folds back progressively with an overshoot-then-settle bounce
    ...
    // front sweeps forward, rear follows, ending fully flat (0-180 degrees)
    ...
    // slow deliberate front stretch, then rear stretch (like a real dog stretch)
    ...
    // a repeating diagonal gait cycle (front/ rear alternate sweeping forward and back)
    void anim_walk(void)
    {
        reset_to_neutral();
    ...
    // a whole choreographed routine: swagger -> rocking lean -> lie down -> floor shimmy -> get back up
    void anim_dance(void)
    {
        ...
        // all animations end with this, cutting PWM signal so servos don't buzz/ jitter or overheat while holding position
        pwm_off_all();
    }